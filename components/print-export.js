...
function appendBaseDatosSheet(wb){
  const bd = [['Capitulo','Codigo','Descripcion','Unidad','Ramo','Materiales','Mano Obra','Equipo','Subcontrato','P. Unitario']];
  ...
  ws['!cols']=[{wch:30},{wch:10},{wch:50},{wch:6},{wch:12},{wch:14},{wch:14},{wch:14},{wch:14},{wch:14}];
...
function appendApuSheet(wb){
  const apuRows=[['ANALISIS DE PRECIOS UNITARIOS'],[''],['Partida','Tipo','Descripcion Insumo','Unidad','Cantidad','Precio Unit.','Subtotal','% del APU']];
  DB.forEach(p=>{
    const ins=APU[p.cod.replace('.','_')]||[];
    if(!ins.length) return;
    const total=ins.reduce((a,i)=>a+i.qty*i.pu,0);
    apuRows.push([`${p.cod} - ${p.desc}`,'','','','','',`₲ ${fmtN(pu(p))}`,'']);
    ins.forEach(i=>{
      const st=i.qty*i.pu;
  ...
  if(apuRows.length>3){
    ...
    ws['!cols']=[{wch:40},{wch:12},{wch:40},{wch:8},{wch:10},{wch:14},{wch:14},{wch:8}];
    XLSX.utils.book_append_sheet(wb, ws, 'APU Detallado');
  }
}
...
function leerArchivoImport(file){
  const r=new FileReader(); r.onload=ev=>{
    try{
      ...
      if(!data.length){ notif('Archivo vacio o invalido', '#E05555'); return; }
      // Detectar fila de encabezado
      let headerRow=0;
      for(let i=0;i<Math.min(5,data.length);i++){
        const row=data[i].join(',').toLowerCase();
        if(row.includes('descripci') || row.includes('codigo') || row.includes('código')){ headerRow=i; break; }
      }
      ...
      info.textContent=`${_importData.length} partidas detectadas en "${file.name}"`;
      let th='<thead><tr><th style="padding:5px 8px;background:var(--bg2);font-size:10px;color:var(--txt3)">Codigo</th><th style="padding:5px 8px;background:var(--bg2);font-size:10px;color:var(--txt3)">Descripcion</th><th style="padding:5px 8px;background:var(--bg2);font-size:10px;color:var(--txt3)">Ud.</th><th style="padding:5px 8px;background:var(--bg2);font-size:10px;color:var(--txt3);text-align:right">P. Unit. ₲</th></tr></thead><tbody>';
      ...
      if(_importData.length>10) th+=`<tr><td colspan="4" style="padding:6px 8px;font-size:10px;color:var(--txt3);text-align:center">... y ${_importData.length-10} mas</td></tr>`;
      th+='</tbody>';
...
function confirmarImport(){
  ...
  _importData.forEach(r=>{
    ...
    if(!existe){
      // Intentar mapear el capitulo por nombre
      let capId='01';
  ...
  notif(`${agregadas} partidas importadas${existentes ? ` (${existentes} ya existian, no se duplicaron)` : ''}`);
}
